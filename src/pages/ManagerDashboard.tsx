...
const ManagerDashboard = () => {
  ...
  const secondaryTabs: { key: ManagerTab; icon: React.ElementType; label: string }[] = [
          ...
          <div className="flex gap-1 bg-muted rounded-xl p-1">
            {primaryTabs.map(({ key, icon: Icon, label }) => (
              <button
                ...
                className={`flex-1 min-w-[3.5rem] flex items-center justify-center gap-1 sm:gap-2 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-semibold transition-colors whitespace-nowrap ${
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center justify-center gap-1 sm:gap-2 py-2 sm:py-2.5 px-3 rounded-lg text-xs sm:text-sm font-semibold transition-colors whitespace-nowrap ${
                  isSecondaryActive ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
                }`}
              >
                <MoreHorizontal size={14} />
                <span className="hidden sm:inline">{isSecondaryActive ? secondaryTabs.find(t => t.key === activeTab)?.label : "עוד"}</span>
              </button>
              {moreOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setMoreOpen(false)} />
                  <div className="absolute left-0 sm:right-0 sm:left-auto top-full mt-1 z-50 bg-card border border-border rounded-xl shadow-lg py-1 min-w-[160px]">
                    {secondaryTabs.map(({ key, icon: Icon, label }) => (
                      <button
                        key={key}
                        onClick={() => { setActiveTab(key); setMoreOpen(false); }}
                        className={`w-full flex items-center gap-2 px-4 py-2.5 text-sm transition-colors ${
                          activeTab === key ? "bg-accent text-accent-foreground font-semibold" : "text-foreground hover:bg-muted"
                        }`}
                      >
                        <Icon size={14} />
                        {label}
